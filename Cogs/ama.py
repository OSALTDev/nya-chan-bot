    ...
    async def processq(self, ctx, timestamp : str = None):
        ...
        async for msg in ctx.channel.history(limit=200):
            ...
            if to_save:
                ...
                cursor.execute("""INSERT INTO messages (id, id_server, id_stream, author, datetime, question, timestamp) VALUES (null, %s, %s, %s, %s, %s, %s)""", (ctx.guild.id, id_stream, q_author, q_date, q_content, q_timestamp))
                connection.commit()
                #await destination.send('From {} - {} UTC (Processed by {})\n--------------------------\n{}'.format(msg.author.mention, msg.created_at.strftime('%c'), ctx.author.mention, msg.content))
                #await msg.delete()
