...
class Giveaway(BaseCog):
    ...
    async def ga(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await bot_channel.send('Invalid giveaway command passed, {}'.format(ctx.author.mention))
    ...
    async def start(self, ctx, giveaway_name: str):
        ...
        if ga_role is not None:
            await bot_channel.send('The giveaway "{}" already exists, {}.'.format(giveaway_name, ctx.author.mention))
            return False
...
this giveaway\n```!n.ga enter {0}``````!n.ga leave {0}```'.format(giveaway_name))
    ...
    async def stop(self, ctx, giveaway_name: str):
        ...
        for x in ctx.guild.roles:
            if x.name == 'giveaway_{}'.format(giveaway_name):
                ga_role = x
        if ga_role is None:
            await bot_channel.send('The giveaway "{}" doesn\'t exist, {}.'.format(giveaway_name, ctx.author.mention))
            return False
    ...
    async def list(self, ctx):
        ...
        if len(ga_roles) == 0:
            await bot_channel.send('There is no active giveaways, {}.'.format(ctx.author.mention))
            return False
        await bot_channel.send('Here is the list of the active giveaways ({}) :\n```{}```'.format(len(ga_roles),
                                                                                                  ", ".join(
                                                                                                      str(x) for x in
                                                                                                      ga_roles)))
    ...
    async def enter(self, ctx, giveaway_name: str):
        ...
        if has_role is not False:
            await bot_channel.send('You already are in the giveaway "{}", {}'.format(giveaway_name, ctx.author.mention))
            return False
        await ctx.author.add_roles(ga_role)
        await bot_channel.send('You just entered the giveaway "{}", {}'.format(giveaway_name, ctx.author.mention))
    ...
    async def leave(self, ctx, giveaway_name: str):
        ...
        if has_role is False:
            await bot_channel.send('You are not in the giveaway "{}", {}'.format(giveaway_name, ctx.author.mention))
            return False
        await ctx.author.remove_roles(ga_role)
        await bot_channel.send('You just left the giveaway "{}", {}'.format(giveaway_name, ctx.author.mention))
    ...
    async def pick(self, ctx, giveaway_name: str):
        ...
        if len(participants) == 0:
            await bot_channel.send('Nobody entered the giveaway "{}", {}.'.format(giveaway_name, ctx.author.mention))
            return False
    ...
    async def winners(self, ctx, giveaway_name: str):
        ...
        if len(winners) == 0:
            ...
            return False
        await bot_channel.send('List of winner :\n{}'.format("\n".join(str(x) for x in winners)))
